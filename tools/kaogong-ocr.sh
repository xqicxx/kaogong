...
while [ $# -gt 0 ]; do
  case "$1" in
    --source)      [ $# -ge 2 ] || { echo "--source 后面要跟讲义名" >&2; exit 2; }; SOURCE="$2"; shift 2 ;;
    --module)      [ $# -ge 2 ] || { echo "--module 后面要跟模块，如 行测/判断推理" >&2; exit 2; }; MODULE="$2"; shift 2 ;;
    --start-page)  [ $# -ge 2 ] || { echo "--start-page 后面要跟页码" >&2; exit 2; }; START="$2"; shift 2 ;;
...
TMP="$(mktemp -d)" || { echo "mktemp 失败" >&2; exit 3; }
...
t_rect=0; t_ocr=0
for f in "${ARGS[@]}"; do
  [ -f "$f" ] || { echo "  ✗ 找不到文件：$f" >&2; FAILED=$(( FAILED + 1 )); continue; }
  name="$(basename "${f%.*}")"
  s=$(date +%s%N)
